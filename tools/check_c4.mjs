...
assert.doesNotMatch(generator, /httpsCallable\(/, "Burst generation must not use the failing callable CORS transport");
assert.match(generator, /fetch\("\/api\/generate-burst"/, "Burst generation must use the same-origin Hosting proxy");
assert.match(generator, /getIdToken\(\)/, "Burst generation must authenticate its HTTPS request");
...
assert.doesNotMatch(firebase, /getFunctions/, "the Firebase client must not retain the unused callable transport");
assert.match(functions, /exports\.generateBurstQuestionHttp/, "the Functions project must expose the direct Burst HTTPS endpoint");
assert.match(functions, /GEMINI_API_KEY/, "fast Burst generation must use a server-side secret");
assert.match(functions, /Access-Control-Allow-Origin/, "the direct endpoint must set CORS headers explicitly");
assert.match(firebaseConfig, /generateBurstQuestionHttp/, "Hosting must proxy Burst generation on the ByteBlitz origin");
